...
function buildFilenameUrlMap(characterId) {
  ...
  const map = {};
  Object.values(listAssets(charAssets)).forEach(path => { map[String(path).split("/").pop()] = assetUrl(path); });
  return map;
}
...
// DEBUG STEP 2C: head_face already contains the ears in the supplied asset.
// head_ears is therefore disabled. Hair remains locked off for independent ear validation.
const HERO_BODY_LAYER_ORDER = ["hips.png", "torso_base.png", "head_face.png"];
...
function useHeroRig(characterId) {
  ...
  useEffect(() => {
    let alive = true;
    loadHeroRig(characterId).then(rig => { if (alive) setState({ ready: true, rig }); });
    return () => { alive = false; };
  }, [characterId]);
  return state;
}
