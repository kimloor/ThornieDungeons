...
function asset(key) {
  ...
  if (!path) { console.warn(`Asset not found in manifest: ${key}`); return ""; }
...
async function loadHeroRig(characterId) {
  ...
  for (const path of rigPathCandidates(characterId)) {
    try { const rig = await loadAssetJSON(path); if (rig && rig.joints && rig.layers && rig.masterCanvas) { console.log(`[HeroRig] Loaded ${characterId} rig from /assets/${path}`); RIG_CACHE[characterId] = rig; return rig; } } catch (e) {}
  }
  RIG_CACHE[characterId] = null; return null;
}
...
function buildFilenameUrlMap(characterId) {
  const charAssets = ASSETS[characterId]; if (!charAssets) return {};
  const map = {}; Object.values(listAssets(charAssets)).forEach(path => { map[String(path).split("/").pop()] = assetUrl(path); }); return map;
}
function resolveLayerPlacement(rig, layerFilename) {
  const layerDef = rig.layers[layerFilename]; if (!layerDef) return null;
  if (layerDef.offset) return { x: layerDef.offset.x, y: layerDef.offset.y };
  const joint = rig.joints[layerDef.joint]; if (!joint || !layerDef.localPivot) return null;
...
// DEBUG STEP 3: enable only the left/right arm chain. Head hair remains disabled until head is locked.
...
function useHeroRig(characterId) {
  const [state, setState] = useState(() => RIG_CACHE[characterId] !== undefined ? { ready: true, rig: RIG_CACHE[characterId] } : { ready: false, rig: null });
  useEffect(() => { let alive = true; loadHeroRig(characterId).then(rig => { if (alive) setState({ ready: true, rig }); }); return () => { alive = false; }; }, [characterId]);
  return state;
}
function HeroModularComposer({ rig, layerOrder, filenameToUrl, canvasWidth, canvasHeight, weaponKey, equipmentKeys, anim = "" }) {
  if (!rig) return null;
