...
const BATTLE_VFX_PRESENTATION = (() => {
  ...
  const SKILL_EFFECTS = Object.freeze({
    power_strike: { effectKey: "slash_basic", kind: "single", placement: "lane" },
    heavy_blow: { effectKey: "slash_heavy", kind: "single", placement: "lane" },
    toxic_strike: { effectKey: "slash_status.toxic", kind: "single", placement: "lane", status: "poison", statusEffectKey: "poison_hit" },
    silent_edge: { effectKey: "slash_status.silence", kind: "single", placement: "lane", status: "silence", statusEffectKey: "silence_hit" },
  ...
  function resolvedEvents(previous, next, actor, command, completedAction = true, options = {}) {
    if (!completedAction || actor?.kind !== "hero") return [];
    ...
    const resolvedBasic = requestedBasic && newLogs.some(entry =>
    ...
    const mapping = skillId === "basic_attack"
      ? (options.basicEffectKey ? { effectKey: options.basicEffectKey, kind: "single", placement: "lane" } : null)
    ...
    const events = mapping.placement === "arena"
      ? [{ effectKey: mapping.effectKey, kind: mapping.kind, placement: mapping.placement, targetId: null, targetIds: unique(targetIds), skillId }]
      : unique(targetIds).map(targetId => ({ effectKey: mapping.effectKey, kind: mapping.kind, placement: mapping.placement, targetId, skillId }));

    if (mapping.status) {
      const appliedTargets = unique(newLogs
        .filter(entry => entry.type === "status" && entry.actorId === actor.id && entry.status === mapping.status)
        .map(entry => entry.targetId));
      appliedTargets.forEach(targetId => events.push({ effectKey: mapping.statusEffectKey, kind: "status", placement: "target", targetId, skillId }));
    }
    return events;
  }
