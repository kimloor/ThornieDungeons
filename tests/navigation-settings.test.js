...
test("shared GameDock owns the only More menu with Settings and Save", () => {
...
test("Character, Inventory, Pet, and Map use shared non-navigating More behavior", () => {
  ...
  assert.match(characterDock, /onSettings[\s\S]*onSave/);
  assert.match(pet, /activeKey: "pets"[\s\S]*onSettings[\s\S]*onSave/);
  assert.match(inventory, /activeKey:"inventory", onSettings, onSave/);
  assert.doesNotMatch(inventory, /onToggleMore:onClose/);
  assert.match(map, /e\(GameDock,[\s\S]*onSettings,[\s\S]*onSave/);
});
