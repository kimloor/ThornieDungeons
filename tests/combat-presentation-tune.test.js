...
test("battle Pet grows from its ground edge and remains restrained on mobile", () => {
  ...
  assert.match(styles, /\.md-pet-slot\s*\{[^}]*left:clamp\(64px,19%,104px\)[^}]*top:75%[^}]*scale\(\.82\)[^}]*transform-origin:center bottom/s);
  assert.match(styles, /@media \(max-width: 380px\)[\s\S]*\.md-pet-slot\s*\{[^}]*scale\(\.8\)/s);
});
